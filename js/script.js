...
function calcularPuntaje() {
  ...
  if (testigosVal === "4") { puntaje += 2; }        // 4 o más testigos
  if (explicacion === "no") { puntaje += 3; }       // sin explicación científica
  ...
  if (puntaje <= 4) {
    ...
    clase          = "debil";
    recomendacion  = "La evidencia disponible es insuficiente para establecer un caso sólido. Se recomienda ampliar la recolección de datos antes de proceder con un informe formal.";
    color          = "#f0c040";
    porcentaje     = Math.max(10, Math.round((puntaje / 14) * 40));
  } else if (puntaje <= 8) {
    nivel          = "Evidencia Moderada";
    clase          = "moderada";
    recomendacion  = "El caso presenta características de interés. Se recomienda escalarlo a análisis secundario y buscar confirmación adicional mediante fuentes independientes.";
    color          = "#ff9900";
    porcentaje     = Math.round(40 + (puntaje / 14) * 35);
  } else {
    nivel          = "Evidencia Fuerte";
    clase          = "fuerte";
    recomendacion  = "ALERTA: Este caso cumple múltiples criterios de alta confiabilidad. Se recomienda apertura de expediente oficial, notificación a autoridades competentes y preservación inmediata de todas las pruebas.";
    color          = "#ff4444";
  ...
  // Animación de barra con pequeño delay
  setTimeout(function () {
